...
class HeteroGNN(Module):
    def __init__(self, metadata, hidden_dim=32, dropout=0.1):
        ...
        # Add another GAT layer to enhance feature extraction capability
...
def get_train_test_set():
    ...
    # Prepare training pairs for recommendation (positive samples)
    ...
    # Ensure that the mapping has no missing values
    logger.info("Checking if all author_idx and unit_idx are valid...")
    if train_positive_pairs['author_idx'].isna().any() or train_positive_pairs['unit_idx'].isna().any():
        logger.error("Error: Found NaN values in author_idx or unit_idx. Please check ID mappings.")
        raise ValueError("Invalid ID mapping in train_positive_pairs.")
    train_positive_pairs.dropna(subset=['author_idx', 'unit_idx'], inplace=True)
    logger.info(f"Valid training pairs count: {len(train_positive_pairs)}")

    # Negative sampling
    logger.info("Performing negative sampling...")
    negative_samples = []
    for idx in range(len(train_positive_pairs)):
        author_id = train_positive_pairs.iloc[idx]['author_id']
        true_unit_id = train_positive_pairs.iloc[idx]['unit_id']
        
        # Randomly sample a negative unit_id
        while True:
            negative_unit_id = random.choice(unit_ids)
            if negative_unit_id != true_unit_id and negative_unit_id in unit_id_map:
                negative_samples.append({'author_id': author_id, 'unit_id': negative_unit_id, 'label': 0})
                break

    # Ensure the negative samples' indices are valid
    negative_df = pd.DataFrame(negative_samples)
    negative_df['author_idx'] = negative_df['author_id'].map(author_id_map)
    negative_df['unit_idx'] = negative_df['unit_id'].map(unit_id_map)

    if negative_df['author_idx'].isna().any() or negative_df['unit_idx'].isna().any():
        logger.error("Error: Found NaN values in author_idx or unit_idx in negative samples. Please check ID mappings.")
        raise ValueError("Invalid ID mapping in negative samples.")
    negative_df.dropna(subset=['author_idx', 'unit_idx'], inplace=True)
    logger.info(f"Valid negative samples count: {len(negative_df)}")

    # Combine positive and negative samples
    train_pairs = pd.concat([train_positive_pairs, negative_df]).reset_index(drop=True)
    logger.info(f"Total training pairs after negative sampling: {len(train_pairs)}")

    # Verify the index columns are created correctly
    logger.info(f"Training pairs sample:\n{train_pairs[['author_id', 'author_idx', 'unit_id', 'unit_idx', 'label']].head()}")
